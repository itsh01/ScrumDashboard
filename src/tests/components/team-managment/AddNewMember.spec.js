define(['React', 'flux/flux', 'constants', 'stubContext', 'components/team-management/AddNewMember'],
    function (React, Flux, constants, stubContext, AddNewMember) {
        ...
        describe('add new member', function () {
            beforeEach(function () {
                localStorage.clear();
                flux = new Flux('https://scrum-dashboard-test.firebaseio.com');
                var AddNewMemberWithContext = stubContext(AddNewMember, {flux: flux, blankMemberSchema: flux.membersStore.getBlankMember()});
